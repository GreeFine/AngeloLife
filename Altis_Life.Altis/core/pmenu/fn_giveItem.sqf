...
_val = ctrlText 2010;
ctrlShow[2002,false];
if((lbCurSel 2023) == -1) exitWith {hintSilent "Aucune personne selectionné";ctrlShow[2002,true];};
_unit = lbData [2023,lbCurSel 2023];
_unit = call compile format["%1",_unit];
if((lbCurSel 2005) == -1) exitWith {hintSilent "Aucun objet selectionné";ctrlShow[2002,true];};
...
if(!([_val] call TON_fnc_isnumber)) exitWith {hintSilent "Vous ne avez pas saisi un format de nombre réel.";ctrlShow[2002,true];};
if(parseNumber(_val) <= 0) exitWith {hintSilent "Vous devez entrer au montant réel que vous voulez donner.";ctrlShow[2002,true];};
