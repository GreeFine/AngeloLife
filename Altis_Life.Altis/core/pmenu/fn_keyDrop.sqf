...
_sel = lbCurSel _list;
if(_sel == -1) exitWith {hintSilent "Aucune donnée sélectionnés";};
_index = (parseNumber(_list lbData _sel));
_vehicle = life_vehicles select _index;
if(_vehicle isKindOf "House_F") exitWith {hintSilent "Vous ne pouvez pas supprimer les clés de votre maison!"};
